...
class _CreateFacturaState extends State<CreateFactura> {
  ...
  void _initializeData() async {
    ...
    final numeroFactura = await _facturaViewModel.generarNumeroFactura();
    _numeroFacturaController.text = numeroFactura;

    // Establecer fechas por defecto
    final ahora = DateTime.now();
    _fechaEmisionController.text = DateFormat('dd/MM/yyyy').format(ahora);
    _fechaVencimientoController.text =
        DateFormat('dd/MM/yyyy').format(ahora.add(const Duration(days: 30)));

    // Impuesto por defecto
    _impuestoController.text = '13.0';
  ...
  void _guardarFactura() async {
    ...
    try {
      final factura = Factura(
        ...
        servicioId:
            '', // Se puede agregar lógica para asociar con un servicio específico
        items: _items,
      ...
      final success = await _facturaViewModel.guardarFactura(factura);

      if (success) {
        // Crear notificación de facturación exitosa
        await NotificacionUtils.crearNotificacionFactura(
          'Factura creada exitosamente',
          'Se ha creado una nueva factura para el cliente ${_clienteSeleccionado?.nombreComercial ?? ""}',
        );
      ...
      FlashMessages.showError(
        context: context,
        message: 'Error al crear la factura: $e',
      );
    }
  }
